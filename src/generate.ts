...
import { piclick } from "monofile-utilities/lib/piclick";
...
json2ts.toLowerFirstLetter = json2ts.toUpperFirstLetter = json2ts.removeMajority = text =>
  text;

export function parseXml() {
  const config = fs.readFileSync(
    __dirname + "/../ta-lib/ta_func_api.xml",
    "utf8"
  );
  parseString(config, (err, result) => {
    if (err) {
      ...
      fs.writeFileSync(
        __dirname + "/ta_func_api.generated.json",
        JSON.stringify(result, void 0, 2)
      );
      const content = json2ts.convertObjectToTsInterfaces(
        result,
        "TaFuncApiXml"
      );
...
export function displayTypes() {
  ...
  config.FinancialFunctions.FinancialFunction.forEach(func => {
    func.RequiredInputArguments &&
      func.RequiredInputArguments.forEach(requires => {
    ...
    func.OptionalInputArguments &&
      func.OptionalInputArguments.forEach(options => {
    ...
    func.OutputArguments &&
      func.OutputArguments.forEach(outputs => {
    ...
    Object.keys(result)
      .map(name => {
        return (
          name +
          ": " +
          Object.keys(result[name as keyof typeof result]).join(", ")
        );
      })
      .join("\n")
  );
...
export function optDft(argv: OptionalInputArgument, ts = false) {
  ...
  return type === "TA_MAType"
    ? ts
      ? TA_MATypes[argv.DefaultValue[0]].split("_")[2]
      : TA_MATypes[argv.DefaultValue[0]]
    : argv.DefaultValue[0];
}
...
export function optType(argv: OptionalInputArgument, ts = false) {
  if (argv.Type.length !== 1) {
    throw new TypeError(
      "opt type must be a single string " + JSON.stringify(argv)
    );
  }
  if (ts) {
    switch (argv.Type[0]) {
      ...
      default:
        throw new TypeError(
          "Unrecognized opt param type " + JSON.stringify(argv)
        );
    }
  }
  switch (argv.Type[0]) {
...
export function outType(argv: OutputArgument) {
  if (argv.Type.length > 1) {
    throw new TypeError(
      "output type must be a single string " + JSON.stringify(argv)
    );
  }
  switch (argv.Type[0]) {
    case "Double Array":
    ...
    default:
      throw new TypeError(
        "unrecognized out param type " + JSON.stringify(argv)
      );
  }
}
...
export const TA_MATypes: Record<string | number, string> = createMap({
  0: "TA_MAType_SMA",
  1: "TA_MAType_EMA",
  2: "TA_MAType_WMA",
  3: "TA_MAType_DEMA",
...
export function generateBindings({ _ }: Arguments) {
  ...
  config.FinancialFunctions.FinancialFunction.filter(
    ...
    const required: RequiredInputArgument[] = func.RequiredInputArguments
      ? func.RequiredInputArguments.reduce(
          (prev, curr) => {
            return curr.RequiredInputArgument
    ...
    const optional: OptionalInputArgument[] = func.OptionalInputArguments
      ? func.OptionalInputArguments.reduce(
          (prev, curr) => {
            return curr.OptionalInputArgument
              ? prev.concat(curr.OptionalInputArgument)
              : prev;
          },
    ...
    const output: OutputArgument[] = func.OutputArguments
      ? func.OutputArguments.reduce(
          (prev, curr) => {
            return curr.OutputArgument
      ...
      : [];
    const doubleRequired = required.filter(
      argv => RecordFields.indexOf(argv.Type[0]) === -1
    );
    const outInitJS: string[] = ["v8::Local<v8::Array> outAll_JS;"];
    output.forEach(argv => {
      outInitJS.push(
        `v8::Local<v8::Array> ${jsOutName(
          argv
        )} = Nan::New<v8::Array>(outLength);`
      );
    });
    if (output.length === 1) {
    ...
    body
      // function declare
      .normal(
        `void TA_FUNC_${name}(const Nan::FunctionCallbackInfo<v8::Value> &info) {`
      )
      // first argument
      .indent(
      ...
      .normal(
        ...required.map(
          argv => `double *${inName(argv)} = new double[inLength];`
        )
      )
      ...
      .indent(
        ...required.map((argv, index) => {
          return `v8::Local<v8::Array> ${jsInName(
            argv
          )} = v8::Local<v8::Array>::Cast(info[${index}]);`;
        })
      )
      // init required
      ...
      .indent(
        ...required.map(
          argv =>
            `${inName(argv)}[i] = Nan::Get(${jsInName(
              argv
            )}, i).ToLocalChecked()->NumberValue(context).FromJust();`
        )
      )
      .undent("}")
      // init optional
      .normal(
        ...optional.map((argv, index) => {
          ...
          return (
            `${optName(
              argv
            )} = argc > ${index} && info[${index}]->Is${check}()` +
            ` ? ${cast} info[${index}]->${check}Value(context).FromJust() : ${optName(
              argv
            )};`
          );
        })
      ...
      .normal(
        ...["startIdx", "endIdx"].map((name, index) => {
      ...
      .normal(
        ...required.map(argv => {
          return `${inName(argv)}[i] = Nan::Get(inObject, ${inName(
            argv
          )}Name).ToLocalChecked()->NumberValue(context).FromJust();`;
        })
      )
      .undent("}")
      .normal(
      ...
      .undent("}")
      .normal(
        `int lookback = TA_${name}_Lookback(${optional
          .map(argv => optName(argv))
          .join(", ")});`
      )
      ...
      .normal(
        ...output.map(
          argv =>
            `${outType(argv)} *${outName(argv)} = new ${outType(
              argv
            )}[outLength];`
        )
      )
      ...
      .indent(
        ...output.map(argv => {
          return `Nan::Set(${jsOutName(
            argv
          )}, i, Nan::New<v8::Number>(${outName(argv)}[i]));`;
        })
      )
      .undent("}")
      .normal(returnStatement)
      ...
      .undent("}", "", "");
    footer.normal(
      `Nan::Set(exports, Nan::New("${name}").ToLocalChecked(), Nan::New<v8::FunctionTemplate>(TA_FUNC_${name})->GetFunction(Nan::GetCurrentContext()).ToLocalChecked());`
    );
  });
  ...
  footer
    ...
    .normal(
      ...Object.keys(TA_MATypes).map(value => {
  ...
  fs.writeFileSync(
    __dirname + "/talib-binding.generated.cc",
    body.build() + footer.build()
  );
}
...
export function generateTypes() {
  ...
  config.FinancialFunctions.FinancialFunction.forEach(func => {
    ...
    body
      .normal("/**", ` * ${name} - ${func.ShortDescription[0]}`, " *")
      .normal(
        ...required.map(argv => {
      ...
      .normal(
        ...optional.map(argv => {
          return ` * @param {${optType(argv, true)}} [${optName(argv)}=${optDft(
            argv,
            true
          )}] - ${argv.ShortDescription[0]}`;
        })
      )
      .normal(
      ...
      .normal(
        output.length === 1
          ? ` * @returns {number[]} - ${output[0].Name[0]} (${
              output[0].Type[0]
            })`
          : ` * @returns {[${output
              .map(() => "number[]")
              .join(", ")}]} - [${output.map(argv => argv.Name[0]).join(", ")}]`
      )
      .normal(" */")
      .normal(
        `export declare function ${name}(` +
          ...
          (required.length > 0 ? ", " : "") +
          optional
            .map(argv => optName(argv) + "?: " + optType(argv, true))
            .join(", ") +
          (optional.length > 0 ? ", " : "") +
          "startIdx?: number, endIdx?: number): " +
          (output.length === 1
            ? "number[]"
            : `[${output.map(() => "number[]").join(", ")}]`) +
          ";"
      )
      .normal(
        "/**",
      ...
      .normal(
        ...doubleRequired.map(argv => {
          return ` * @param {string} ${inName(
            argv
          )}Name - The field name to extract from \`inRecords\``;
        })
      )
      .normal(
        ...optional.map(argv => {
...
export function generateGyp() {
  const sources = G.sync(
    "ta-lib/c/src/{ta_abstract,ta_common,ta_func}/**/!(ta_java_defs|excel_glue).c",
    {
      cwd: __dirname + "/..",
      nodir: true
    }
  );
  const includes = G.sync(
    "ta-lib/c/{include,src/{ta_abstract,ta_common,ta_func}/**}/",
    {
      cwd: __dirname + "/..",
      nodir: false
    }
